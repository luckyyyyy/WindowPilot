...
从 1.4 起支持自动更新：默认每天检查 GitHub Releases，在后台下载并于退出时安装。菜单栏的“检查更新…”可立即更新，“设置 → 关于”可关闭自动检查或自动安装。1.3 及更早版本需要手动安装一次新版。更新包和更新列表均验证 EdDSA 签名。已实测从 1.4.0 后台下载、安装并重启到 1.4.1，授权与设置保留，详见 [验证记录](docs/VALIDATION.md)。
...
- **鼠标悬停反馈**：指针经过窗口行时显示中性灰色背景，和键盘选中的主色高亮清楚区分；点击即可切换。
- **恢复最小化窗口**：列表显示最小化状态，选择后自动恢复；隐藏应用也可切换。
- **管理选中窗口**：非搜索状态下按 ⌘W 关闭窗口、⌘Q 请求退出所属应用，保留原应用的保存与取消流程。
- **按 X 搜索**：先按 `x` 进入搜索，再输入关键词（例如 `xq` 搜索 QQ，前缀 `x` 不计入搜索词）。搜索中即使按住 ⌘，`w` / `q` 也只输入文字；删空关键词仍保持搜索模式，Esc 取消。支持忽略大小写的模糊匹配（如 `ws` → `watchOptions`、`gc` → `Google Chrome`）、Unicode 和多个关键词；按匹配程度排序并高亮命中文字。
- **按需过滤**：选择是否显示最小化、隐藏和无窗口应用；按应用及标题添加排除规则。
- **自定义快捷键**：在“设置 → 快捷键”点击按键组合录制，支持全局、同应用、关闭、退出与进入搜索。额外按 ⇧ 反向切换，松开所选修饰键确认；录制时不会触发原快捷键，支持冲突提示、取消与恢复默认。
...
扫描和切换使用独立队列，按键不等待整轮扫描。AX 通知按应用合并增量刷新，定时完整校准；超时应用保留缓存并退避重试。自身窗口始终在主线程通过 AppKit 操作，避免同进程 AX 回调引起崩溃。详见[架构](docs/ARCHITECTURE.md)和[验证范围](docs/VALIDATION.md)。
